...
.VERSION
    0.1 - Initial Version for reporting Lync/Skype4B Users which reported with Poor Call quality Calls
...
'@	
		...
		[void]$sb.AppendLine("<tr><td colspan='3'><strong><center>Users Reported with Poor Call for $StartDate</cenetr></strong></td></tr>")
		[void]$sb.AppendLine("<tr>")
		[void]$sb.AppendLine("<td><strong>Call Date</strong></td>")
		[void]$sb.AppendLine("<td><strong>CallerUri</strong></td>")
		[void]$sb.AppendLine("<td><strong>Number of Poor Calls</strong></td>")
		[void]$sb.AppendLine("</tr>")
	}
...
$StartDate = (get-date -Format d (get-date).AddDays(-1)).toString()
...
ORDER  BY CallDate, NumberOfPoorCalls DESC" 
...
if (($From.Length -gt 0) -AND ($To.Length -gt 0) -AND ($SmtpServer.Length -gt 0))
{
    ...
    -From $From `
    -Subject "Reported Users with Poor Calls for $StartDate" `
