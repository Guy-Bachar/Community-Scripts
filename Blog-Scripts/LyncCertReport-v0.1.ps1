...
$serverlist = Get-CsPool | Where-Object {($_.Services -like "*Registrar*") -OR ($_.Services -like "*EDGE*") -OR ($_.Services -like "*WAC*")}
...
Add-Content $fileName "<td width='10%' align='center'><font color=#FFFFFF>Friendly Name</font></td>"
...
foreach ($server in $serverlist)
{
    ...
    {
        $FQDN = $server.Fqdn
        $Store = New-Object System.Security.Cryptography.X509Certificates.X509Store("$FQDN\MY","LocalMachine") -ErrorAction Stop
        $Store.Open("ReadOnly")
    ...
    catch
    {
        ...
        Add-Content $ServicesFileName "<td width='100%' align='center' colSpan=7><font face='segoe ui' color='#FFFFFF' size='2'>$FQDN</font></td>"
